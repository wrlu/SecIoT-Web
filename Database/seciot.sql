...
-- 数据导出被取消选择。
-- 导出  表 seciot.fw_library_risk 结构
CREATE TABLE IF NOT EXISTS `fw_library_risk` (
  `id` int(11) NOT NULL,
  `name` varchar(50) DEFAULT NULL,
  `version` varchar(50) DEFAULT NULL,
  `cve_num` varchar(50) NOT NULL,
  PRIMARY KEY (`id`),
  KEY `FK_fw_library_risk_cve` (`cve_num`),
  KEY `FK_fw_library_risk_fw_third_part_library` (`name`),
  CONSTRAINT `FK_fw_library_risk_cve` FOREIGN KEY (`cve_num`) REFERENCES `cve` (`cve_num`),
  CONSTRAINT `FK_fw_library_risk_fw_third_part_library` FOREIGN KEY (`name`) REFERENCES `fw_third_part_library` (`name`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='保存某个库按版本号的漏洞情况';

-- 数据导出被取消选择。
-- 导出  表 seciot.fw_third_part_library 结构
CREATE TABLE IF NOT EXISTS `fw_third_part_library` (
  `id` int(11) NOT NULL,
  `name` varchar(50) DEFAULT NULL,
