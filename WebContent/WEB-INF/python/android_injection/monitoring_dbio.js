setImmediate(function() {
    Java.perform(function () {
        ...
        SQLiteOpenHelper.getReadableDatabase.overload().implementation = function () {
            ...
            send('[数据库I/O检测] 类型: 数据库只读打开(ReadableDatabase), 数据库文件路径: '+path+', 数据库版本: '+version);
        ...
        SQLiteOpenHelper.getWritableDatabase.overload().implementation = function () {
            ...
            var path = sqlite_database.getPath();
            var version = sqlite_database.getVersion();
            send('[数据库I/O检测] 类型: 数据库读写打开(WritableDatabase), 数据库文件路径: '+path+', 数据库版本: '+version);
            return sqlite_database;
        }
        ...
        SQLiteDatabase.execSQL.overload('java.lang.String').implementation = function (sql) {
            ...
            send('[数据库I/O检测] 类型: 数据库SQL语句执行(execSQL), SQL语句: '+sql+', 数据库文件路径: '+path+', 数据库版本: '+version);
            this.execSQL(sql);
        }
