setImmediate(function() {
    Java.perform(function () {
        ...
        FileInputStream.$init.overload('java.io.File').implementation = function (file) {
            ...
            send('[Host'+localhost+' - 文件I/O检测] 类型: 文件读取(FileInputStream), 文件名: '+file_name+', 文件完整路径: '+file_path);
            return this.$init(file);
        }
        FileInputStream.$init.overload('java.lang.String').implementation = function (name) {
            send('[Host'+localhost+' - 文件I/O检测] 类型: 文件读取(FileInputStream), 文件相对路径: '+name);
        ...
        FileOutputStream.$init.overload('java.io.File').implementation = function (file) {
            var file_name = file.getName();
            var file_path = file.getAbsolutePath();
            send('[Host'+localhost+' - 文件I/O检测] 类型: 文件写入(FileOutputStream), 文件名: '+file_name+', 文件完整路径: '+file_path);
            return this.$init(file);
        }
        FileOutputStream.$init.overload('java.lang.String').implementation = function (name) {
            send('[Host'+localhost+' - 文件I/O检测] 类型: 文件写入(FileOutputStream), 文件相对路径: '+name);
            return this.$init(name);
        }
        ...
        FileReader.$init.overload('java.io.File').implementation = function (file) {
            var file_name = file.getName();
            var file_path = file.getAbsolutePath();
            send('[Host'+localhost+' - 文件I/O检测] 类型: 文件读取(FileReader), 文件名: '+file_name+', 文件完整路径: '+file_path);
            return this.$init(file);
        }
        FileReader.$init.overload('java.lang.String').implementation = function (name) {
            send('[Host'+localhost+' - 文件I/O检测] 类型: 文件读取(FileReader), 文件相对路径: '+name);
            return this.$init(name);
        }
        ...
        FileWriter.$init.overload('java.lang.String').implementation = function (name) {
            send('[Host'+localhost+' - 文件I/O检测] 类型: 文件写入(FileWriter), 文件相对路径: '+name);
            return this.$init(name);
        }
